...
public final class TutorialGraphicalPlayer {
    ...
    public void fillTutorialHandler(TutorialHandler tutorialHandler) {
        ...
        tutorialHandlerProperty.set(tutorialHandler);
    }

    public void updateTutorialText(String nextMessage) {
        tutorialText.clear();
        tutorialText.add(new Text(nextMessage));
        tutorialHandlerProperty.set(null);
    }
    ...
    private final ObservableList<Text> tutorialText;
     ...
     */
    public TutorialGraphicalPlayer(PlayerId playerId, Map<PlayerId, String> playerNames) {
        assert isFxApplicationThread();

        observableGameState = new ObservableGameState(playerId);
        gameMessages = FXCollections.observableArrayList();
        tutorialText = FXCollections.observableArrayList();
     ...
     */
    public void chooseClaimCards(List<SortedBag<Card>> initialClaimCards, ActionHandlers.ChooseCardsHandler chooseCardsHandler) {
        assert isFxApplicationThread();
    ...
    private void createCardsStage(boolean buttonCanBeDisabled, Text text, List<SortedBag<Card>> cards,
        ...
        cardsView.setCellFactory(v -> new TextFieldListCell<>(new TutorialGraphicalPlayer.CardBagStringConverter()));
        Button button = new Button(CHOOSE);
